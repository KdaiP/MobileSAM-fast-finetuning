...
# from monai: https://github.com/Project-MONAI/MONAI
class DiceLoss(nn.Module):
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        if self.to_onehot_y:
            if n_pred_ch == 1:
                ...
                target = self.one_hot(target, num_classes=n_pred_ch)
    ...
    def one_hot(self, labels: torch.Tensor, num_classes: int, dtype: torch.dtype = torch.float, dim: int = 1) -> torch.Tensor:
        """
